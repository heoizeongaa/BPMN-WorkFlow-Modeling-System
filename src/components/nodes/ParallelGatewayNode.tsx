...
export function ParallelGatewayNode({ data: _data, selected }: NodeProps<BpmnNode>) {
  const size = 50;
  const half = size / 2;
    ...
    <div className={`bpmn-node--gateway${selected ? ' bpmn-node--selected' : ''}`}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        <polygon
          ...
          fill="var(--bpmn--parallel--bg)"
          stroke="var(--bpmn--parallel--color)"
          strokeWidth={2}
        />
        <line x1={half} y1={16} x2={half} y2={34} stroke="var(--bpmn--parallel--color)" strokeWidth={2.5} />
        <line x1={16} y1={half} x2={34} y2={half} stroke="var(--bpmn--parallel--color)" strokeWidth={2.5} />
      </svg>
